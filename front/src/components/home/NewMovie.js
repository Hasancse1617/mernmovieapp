...
import { fetchNewMovies } from "../../store/actions/AllMovieAction";
import OwlCarousel from 'react-owl-carousel';
import { NavLink } from 'react-router-dom';

const NewMovie = () => {
    const dispatch = useDispatch();
    const { newMovies } = useSelector((state)=>state.AllMovieReducer);
    useEffect(()=>{
    ...
    return (
        <>
           <div className="home home--static">
                {/* <div className=" owl-carousel" id="flixtv-hero"> */}
                    <OwlCarousel
                       items= {3}
                       className="home__carousel owl-theme" id="flixtv-hero"
                       mouseDrag= {true}
                       touchDrag= {true}
                       dots= {false}
                       loop= {true}
                    ...
                    >
                         {
                             newMovies.map((newmovie)=>(
                                <div key={newmovie._id} className="home__card">
                                    <NavLink to={`/video/${newmovie._id}`}>
                                        <img src={`http://localhost:5000/images/video_thumbnails/${newmovie.thumbnail}`} height={300} alt=""/>
                                    </NavLink>
                                    <div>
                                        <NavLink to={`/video/${newmovie._id}`}><h2>{ newmovie.title }</h2></NavLink>
                                        <ul>
                                            <li>{ newmovie.price_status }</li>
                                            <li>{ newmovie.tags_id[0].name }</li>
                                            <li>{ new Date(newmovie.release_date).getFullYear()}</li>
                                        </ul>
                                    </div>
